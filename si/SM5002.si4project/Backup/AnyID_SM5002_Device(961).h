...
#define DEVICE_INFO_IDLE_TICK       2000        //10s内没有任何状态发生变化，也需要增加一条记录
...
#define DEVICE_SMOKE_ALARM_VALUE        0       //正常状态是0  报警状态是1
#define DEVICE_IN_GATE_0_MSK            0x0111
#define DEVICE_IN_GATE_1_MSK            0x0222
...
typedef struct deviceChargeInfo{
    u8 state;
}DEVICE_CHAGINFO;

typedef struct deviceBatInfo{
    u8 state;
}DEVICE_BATINFO;

#define DEVICE_CTRL_DOOR_TICK           100
...
typedef struct deviceOpInfo{
    ...
    DEVICE_CHAGINFO chargeInfo[DEVICE_SENSOR_NUM];
    DEVICE_BATINFO batInfo[DEVICE_SENSOR_NUM];
    u32 rcdIdleTick[DEVICE_SENSOR_NUM];
    DEVICE_CTRLDOOR ctrlDoor[DEVICE_SENSOR_NUM];
}DEVICE_OPINFO;
