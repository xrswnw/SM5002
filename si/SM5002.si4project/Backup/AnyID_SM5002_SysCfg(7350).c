...
void Sys_BatTask(void)
{
    ...
    if(Uart_IsRcvFrame(g_sBatRcvFrame))
    {
        if(g_sBatRcvFrame.len >= BAT_FRAME_MIN_LEN)
        {
            u16 startPos = 0;
            if(Bat_CheckFrame(&g_sBatRcvFrame, &startPos))//, g_sBatOpInfo.op[g_sBatOpInfo.index], &g_sDeviceOpInfo.batInfo[0].info)
            {
                g_sBatOpInfo.state = BAT_OP_STAT_RX | BAT_OP_STAT_NEXT;
            }
        }
        Uart_ResetFrame(g_sBatRcvFrame);
        if(g_sBatOpInfo.state == BAT_OP_STAT_WAIT)  //如果没有获取想要的响应帧，继续接收数据
        {
            Bat_EnableRxDma();
        }
    }
    
    if(g_sBatOpInfo.state == BAT_OP_STAT_IDLE)  //空闲状态
    {
        if(g_sBatOpInfo.tick + BAT_CHK_TIME < g_nSysTick)
        {
            Bat_GetOp(&g_sBatOpInfo);
            g_sBatOpInfo.state = BAT_OP_STAT_TX;
        }
    }

    if(g_sBatOpInfo.state == BAT_OP_STAT_TX)
    {
        if(g_sBatOpInfo.index < g_sBatOpInfo.num)
        {
            Bat_TxCmd(&g_sBatTxFrame, g_sBatOpInfo.op[g_sBatOpInfo.index]);
            g_sBatOpInfo.tick = g_nSysTick;
            g_sBatOpInfo.state = BAT_OP_STAT_WAIT;
        }
    }

    if(g_sBatOpInfo.state == BAT_OP_STAT_WAIT)
    {
        if(g_sBatOpInfo.tick + UART_TX_TIMEOUT < g_nSysTick)
        {
            g_sBatOpInfo.state = BAT_OP_STAT_TO | BAT_OP_STAT_NEXT;
            Bat_DisableTxDma();
            Bat_DisableRxDma();
        }
    }

    if(a_CheckStateBit(g_sBatOpInfo.state, BAT_OP_STAT_NEXT))
    {
        a_ClearStateBit(g_sBatOpInfo.state, BAT_OP_STAT_NEXT);
        g_sBatOpInfo.state = Bat_GetNext(&g_sBatOpInfo);
    }
}
