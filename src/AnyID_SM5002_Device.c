...
{
    //充电机
    g_sChagOpInfo[index].mode = CHAG_MODE_INIT;
    g_sChagOpInfo[index].pChagParams = &g_sDeviceParams.chagParams;
    Chag_GetOp(&g_sChagOpInfo[index]);
    
    //电池
    g_sBatOpInfo[index].mode = BAT_MODE_GET_INFO;
    Bat_GetOp(&g_sBatOpInfo[index]);

    g_sDeviceGateInfo[index].pBatOpInfo = &g_sBatOpInfo[index];
    g_sDeviceGateInfo[index].pChagOpInfo = &g_sChagOpInfo[index];
    g_sDeviceGateInfo[index].interVal = DEVICE_IVT_TIME_MID;
    g_sDeviceGateInfo[index].rcdIdleTick = DEVICE_INFO_IDLE_TICK_CHAG;
    
    g_sDeviceGateInfo[index].mode = DEVICE_GATE_MODE_STARTUP;
}
        ...
        case UART_FRAME_CMD_SET_OUTINFO:
            if(paramsLen == 2)
            {
                ...
                if(index < DEVICE_GATE_NUM && 
                   (g_sDeviceGateInfo[index].mode == DEVICE_GATE_MODE_IDLE || g_sDeviceGateInfo[index].mode == DEVICE_GATE_MODE_CHARGE))   //只有空闲和充电模式才允许操作
                {
                    if(pParams[1] & DEVICE_OUT_CTRL_POS_DOOR)
                    {
                        ...
                        if(g_sDeviceGateInfo[index].mode == DEVICE_GATE_MODE_CHARGE)    //如果是充电模式，开仓一定要先停止充电
                        {
            ...
            if(paramsLen == 1 + BAT_SN_LEN)
            {
                ...
                if(index < DEVICE_GATE_NUM)
                {
                         ...
                         memcmp(g_nBatBoardcastSn, pParams + 1, BAT_SN_LEN) == 0))
                    {
                        ...
                        g_sDeviceGateInfo[index].brwOpenDoorRpt = 0;
...
void Device_StopCharge(DEVICE_GATEINFO *pGateInfo, u8 index)
{
    ...
    pBatOpInfo->batInfo.state &= (~BAT_STAT_CHARGE);
    if(pBatOpInfo->batInfo.status.volLev == 100 || pBatOpInfo->batInfo.status.volLev > g_sDeviceParams.chagParams.fulVolLev)    //这里用>号，而不是>=号，就是需要多充点1%
    {
        pBatOpInfo->batInfo.state |= BAT_STAT_FULL;
    }
...
void Device_WaitBatIn(BAT_OPINFO *pBatOpInfo, CHAG_OPINFO *pChagOpInfo, u8 index)
{
...
void Device_WaitBatOut(BAT_OPINFO *pBatOpInfo, CHAG_OPINFO *pChagOpInfo, u8 index)
{
    pBatOpInfo->mode = BAT_MODE_GET_INFO;
    Bat_GetOp(pBatOpInfo);
...
void Device_GateStartup(DEVICE_GATEINFO *pGateInfo, u8 index)
{
    if(Chag_IsExist(pGateInfo->pChagOpInfo))              	//充电机和电池都在位，并且正常，这里还要判断消防和门等状态是否正常！！！！！！
    {
        if(Bat_IsExist(pGateInfo->pBatOpInfo))
        {
            ...
            {
                Device_StopCharge(pGateInfo, index);		//停止充电
            }
            else
            {
                Device_StartCharge(pGateInfo, index);		//开始充电
            }
        }
        else 
        {
        	if(pGateInfo->pChagOpInfo->activeBatTime + CHAG_ACTIVE_BAT_TIM < g_nSysTick)
        	{
            	Device_StopCharge(pGateInfo, index);		//超过10s，没有发现电池
    		}
			else
			{
				Device_WaitBatIn(pGateInfo->pBatOpInfo, pGateInfo->pChagOpInfo, index);
			}
        }
    }
    else
    { 
        Device_StopCharge(pGateInfo, index);				//充电机不存在
    }
}
...
void Device_GateReturnBat(DEVICE_GATEINFO *pGateInfo, u8 index)
{
    ...
    {
        if( pGateInfo->sensorState.door == DEVICE_DOOR_CLOSE || pGateInfo->ctrlDoor.bOpen|| pGateInfo->sensorState.rfid == DEVICE_RFID_CLOSE)       //  开锁未完成 pGateInfo->sensorState.rfid == DEVICE_RFID_CLOSE || 
        {
...
void Device_GateBorrowBat(DEVICE_GATEINFO *pGateInfo, u8 index)
{
    ...
    {
        if(pGateInfo->sensorState.door == DEVICE_DOOR_CLOSE || pGateInfo->ctrlDoor.bOpen || pGateInfo->sensorState.rfid == DEVICE_RFID_CLOSE)   //开锁未完成 || 
        {
        ...
        else 
        {
            ...
            Device_WaitBatOut(pGateInfo->pBatOpInfo, pGateInfo->pChagOpInfo, index);
        }
    }
    else if(pGateInfo->brwBatStep == DEVICE_BRWBAT_STEP_CHKBAT)                             //等待电池
    {
        ...
        else
        {
            ...
            if(Bat_IsExist(pGateInfo->pBatOpInfo))
            {
                ...
                {
                    pGateInfo->brwOpenDoorRpt++;
                    if(pGateInfo->brwOpenDoorRpt < DEVICE_BRW_BAT_CHK_RPT)
                    {
                        Device_OpBatRspFrame(UART_FRAME_CMD_BRWBAT, DEVICE_BRWBAT_STEP_CHKBAT, DEVICE_BRW_CHKBAT_ERR_BATEXIST); //电池未拿走
                        //这里加一个延时------------------------------------------------------------------
                        WDG_FeedIWDog();Device_Delayms(500);WDG_FeedIWDog();Device_Delayms(500);WDG_FeedIWDog();Device_Delayms(500);WDG_FeedIWDog();Device_Delayms(500);
                        //重新弹开门
